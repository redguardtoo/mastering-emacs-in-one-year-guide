#+TITLE: Practical guide to use Gnus with Gmail
...
- [[#advanced-tips][Advanced tips]]
  ...
  - [[#its-recommended-to-subscribe-gmailsent-mail-folder][It's recommended to subscribe "[Gmail]/Sent Mail" folder]]
...
** Usage
After starting Gnus by =M-x gnus=, you come to the "Group Buffer". The "Group Buffer" lists the mail folders. In Gnus, the folder is called "Group". Please the folders are *invisible* in Group Buttfer by default!

You need subscribe the groups to make it visible. For example, INBOX is named "nnimap+gmail:INBOX" in "Group Buffer" and *it's invisible* at the beginning! It's confusing for users.
...
An opened mail folder is called "Summary Buffer" which is, as I said before, lists all mails .
...
That's all you need to know. You could go to section "My =~/
* Essential 5%
It's waste of time to memorize keybindings because I provide *my [[https://github.com/abo-abo/hydra][Hydra]] setup* in the section "Use Hydra to avoid remembering key bindings".
** Search mails
...
For example, query =TEXT github SINCE 1-Jan-2016 FROM chenbin= searches mails are sent by me and contains keyword =github= on year 2016.

People who dislike "friendly" RFC3501 could use [[https://support.google.com/mail/answer/7190?hl=en][Gmail]] syntax([[https://github.com/rhashimoto][Roy Hashimoto]] provides this tip),
#+begin_src elisp
...
** Microsoft Outlook
If syour Exchange Server is not using standard protocol like IMAP or you can't access IMAP port behind firewall, you need [[http://davmail.sourceforge.net/][DavMail]], a "POP/IMAP/SMTP/CalDAV/CardDAV/LDAP exchange gateway".
...
Easiest solution is to switch the FROM field dynamically by =M-x toggle-mail-from-field= (bleow):
#+begin_src elisp
;; Please note below code is not included in sample .gnus.el
...
Please use [[https://github.com/org-mime/][org-mime]] which supports Emacs 24+. It's also the offical version of =org-mime=.
** Read HTML mail
...
To view/download embedded video/audio in feed, you can use mplayer if its support for network streaming is enabled. It's should be enabled by default at Linux/macOS/Windows.

There are also CLI tricks to open images at different OS. See my code for details.
