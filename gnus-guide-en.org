...
But many people get lost in its endless features. It would be much easier if they had learned the *essential 5% features* and ignored the remaining 95% features at the beginning.
...
Before starting Gnus, you *must* create =.gnus.el=. Here is [[#my-gnusel][my "~/.gnus.el"]].
** Usage
...
* My "~/.gnus.el"
Before creating =.gnus.el=, you might need
#+BEGIN_SRC emacs-lisp
...
Login and password is stored in =~/.authinfo.gpg=.
...
You could use =~/.authoinfo= instead. But it's a plain text file. It's not wise to store your credentials into a text file without encryption.

Multiple mail accounts share one =.authinfo.gpg=,

Here is sample =.authinfo.gpg=,
#+BEGIN_SRC conf
