...
** 简介
成为高手很容易.我初学Emacs时还忘记"退出"的快捷键,一年后我[[https://github.com/redguardtoo][跨入高手行列]].
...
- 如果你真下定决心,考虑到Purcell的天赋和勤奋,追赶他的最好办法只有加入他
...
例如,默认在Emacs中移动子窗口焦点不是很方便.需按 =C-x O= 多次.我找到了emacs插件[[https://github.com/dimitri/switch-window][switch-window]],只要按 =C-x O= 一次,会有提示子窗口编号,接下来输入编号就可以了.但还有改善空间,我又找到了[[https://github.com/nschum/window-numbering.el][window-number.el]],只要按 =M-NUM= 一次.
...
** 使用Evil
Evil是[[https://bitbucket.org/lyro/evil/wiki/Home][Vim模拟器]].
...
我的重点是展示一些高级技巧(有些技巧是我独创的),以说明Emacs给我真正的自由.

我引以为豪自己使用Emacs的方式和其他Emacs或者Vim用户完全不同.
*** Text Object
